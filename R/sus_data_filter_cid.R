...
.filter_cid_tibble_internal <- function(
  df,
  disease_group,
  icd_codes,
  icd_column,
  ...
  if (!is.null(disease_group)) {
    ...
    invalid_groups <- disease_group[
      !disease_group %in% names(.icd_disease_groups)
    ]
    ...
    if (length(invalid_groups) > 0) {
      ...
      msg <- list(
        ...
        es = paste0(
          "Grupo(s) de enfermedad invalido(s): '",
          paste(invalid_groups, collapse = "', '"),
          "'.\n",
          "Grupos disponibles: ",
          available_groups
      ...
      if (length(invalid_groups) > 0) {
        ...
        for (inv_group in invalid_groups) {
          ...
          if (min_distance <= 3) {
            suggestions <- c(
              suggestions,
              paste0("'", inv_group, "' -> Did you mean '", closest_match, "'?")
            )
          }
        }
    ...
    if (verbose) {
      msg <- list(
        ...
        pt = paste0(
          "Usando grupo(s) de doenca: ",
          paste(disease_group, collapse = ", "),
          "\n",
          "Total de codigos CID: ",
          length(icd_codes)
        ),
        es = paste0(
          "Usando grupo(s) de enfermedad: ",
          paste(disease_group, collapse = ", "),
          "\n",
          "Total de codigos CIE: ",
          length(icd_codes)
        )
      )
  ...
  if (is.null(icd_column)) {
    ...
    if (is.null(icd_column) || is.na(icd_column)) {
      if (lang == "en") {
        icd_pattern_cols <- grep(
          "underlying_cause|underlying_cause_original|primary_diagnosis|secondary_diagnosis|primary_icd|secondary_icd|maternal_cause",
          names(df),
          value = TRUE,
          ignore.case = TRUE
        )
        ...
        icd_pattern_cols <- grep(
          "causa_basica|causa_basica_original|diagnostico_principal|diagnostico_secundario|cid_principal|cid_secundario|causa_materna",
          names(df),
          value = TRUE,
          ignore.case = TRUE
        )
        ...
        icd_pattern_cols <- grep(
          "causa_basica|causa_basica_original|diagnostico_principal|diagnostico_secundario|cie_principal|cie_secundario|causa_materna",
          names(df),
          value = TRUE,
          ignore.case = TRUE
        )
...
.filter_cid_arrow_internal <- function(
  df,
  icd_codes,
  disease_group,
  icd_column,
  ...
  if (backend_type == "unsupported") {
    cli::cli_alert_danger(
      "Input must be a data.frame, Arrow Dataset, or DuckDB table."
    )
  ...
  if (inherits(df, "climasus_df") || inherits(df, "climasus_dataset")) {
    ...
    detected_system <- tryCatch(sus_meta(df, "system"), error = function(e) {
      NULL
    })
    ...
    if (
      ...
      msg_error <- list(
        en = paste0(
          "Data must be standardized before disease filtering (CID-10). Current stage: ",
          current_stage
        ),
        pt = paste0(
          "Dados devem ser padronizados antes da filtragem por doenca (CID-10). Estagio atual: ",
          current_stage
        ),
        es = paste0(
          "Los datos deben estar estandarizados antes del filtrado por enfermedad (CIE-10). Etapa actual: ",
          current_stage
        )
      )
      cli::cli_abort(msg_error[[lang]])
  ...
  if (!is.null(disease_group)) {
    # Handle single or multiple disease groups
    disease_groups_vec <- if (length(disease_group) == 1) {
      disease_group
    } else {
      disease_group
    }
    ...
    # Validate ALL disease groups exist
    invalid_groups <- disease_groups_vec[
      !disease_groups_vec %in% names(.icd_disease_groups)
    ]
    ...
    if (verbose) {
      msg <- list(
        en = paste0(
          "Using disease group(s): ",
          paste(disease_groups_vec, collapse = ", "),
          "\nTotal ICD codes: ",
          length(icd_codes)
        ),
        pt = paste0(
...
list_disease_groups <- function(
  ...
  if (!lang %in% c("en", "pt", "es")) {
    cli::cli_alert_warning(
      "Language '{lang}' not supported. Using English (en)."
    )
    lang <- "en"
  }
  ...
  if (climate_sensitive_only) {
    all_groups <- all_groups[sapply(.icd_disease_groups, function(x) {
      x$climate_sensitive
    })]
  }
  ...
  result <- data.frame(
    ...
    label = sapply(all_groups, function(g) {
      .icd_disease_groups[[g]]$label[[lang]]
    }),
    climate_sensitive = sapply(all_groups, function(g) {
      .icd_disease_groups[[g]]$climate_sensitive
    }),
    n_codes = sapply(all_groups, function(g) {
      length(.icd_disease_groups[[g]]$codes)
    }),
...
get_disease_group_details <- function(group_name, lang = "en") {
  ...
  msg_climate <- list(
    en = paste0(
      "Climate-sensitive: ",
      ifelse(info$climate_sensitive, "Yes", "No")
    ),
    pt = paste0(
      "Sensivel ao clima: ",
      ifelse(info$climate_sensitive, "Sim", "Nao")
    ),
    es = paste0(
      "Sensible al clima: ",
      ifelse(info$climate_sensitive, "Si", "No")
    )
  )
  cli::cli_alert_info(msg_climate[[lang]])
  ...
  if (info$climate_sensitive && !is.null(info$climate_factors)) {
    msg_factors <- list(
      en = paste0(
        "Climate factors: ",
        paste(info$climate_factors, collapse = ", ")
      ),
      pt = paste0(
        "Fatores climaticos: ",
        paste(info$climate_factors, collapse = ", ")
...
process_icd_codes <- function(codes) {
  ...
  for (code in codes) {
    if (grepl("-", code)) {
      ...
      start_num <- if (nchar(start_num_str) > 0) {
        as.numeric(start_num_str)
      } else {
        0
      }
...
detect_health_system_lazy <- function(df, col_names, backend_type) {
  ...
  if (any(sapply(sim_patterns, function(p) any(grepl(p, col_names_lower))))) {
    ...
    any(sapply(sih_patterns, function(p) any(grepl(p, col_names_lower))))
  ) {
    return("SIH")
  } else if (
    any(sapply(sinan_patterns, function(p) any(grepl(p, col_names_lower))))
  ) {
    return("SINAN")
  } else if (
    any(sapply(sinasc_patterns, function(p) any(grepl(p, col_names_lower))))
  ) {
    return("SINASC")
  }
...
apply_filter_by_backend <- function(df, filter_expr, backend_type) {
  ...
  if (backend_type == "dataframe") {
    ...
    if (filter_expr$type == "exact") {
      ...
      result <- df[
        grepl(filter_expr$pattern, df[[icd_col]], ignore.case = FALSE),
      ]
    }
  } else if (backend_type == "arrow") {
