  ...
  #'   Examples:
  #'   \itemize{
  #'     \item Single variable: `target_var = "tair_dry_bulb_c"`
  #'     \item Multiple variables: `target_var = c("tair_dry_bulb_c", "rh_pct", "ws_2_m_s")`
  #'     \item All numeric variables: `target_var = "all"`
  #'   }
  #'
  #' @param datetime_col
  ...
  #'   \itemize{
  #'     \item `0.3` (30%) - Conservative, high-quality stations only
  #'     \item `0.4` (40%) - Default, balanced approach
  #'     \item `0.6` (60%) - Lenient, includes stations with significant gaps
  #'   }
  ...
  #'
  #' @param workers
  #'   Integer. Number of parallel workers. If `NULL`, uses `max(1, availableCores() - 1)`.
  #'   The value is resolved **once** at the start and reused across all variable iterations,
  #'   avoiding repeated plan switches.
  #'
  #' @param verbose
  ...
  #' \itemize{
  #'   \item **Single variable**: a `tibble` (same class as input) with imputed values in
  #'     `target_var` and an `is_imputed_<var>` flag column.
  #'   \item **Multiple variables / `"all"`**: the same tibble with all requested variables
  #'     imputed sequentially, each with its own `is_imputed_<var>` flag column.
  #'   \item A `sus_meta` attribute records all imputed variables and their rates.
  #' }
  #'
  ...
  sus_climate_fill_inmet_test <- function(
    ...
    # ===========================================================
    # KNOWN CLIMATE VARIABLES (canonical set)
    # ===========================================================
    .known_climate_vars <- c(
      "patm_mb",
      "patm_max_mb",
      "patm_min_mb",
      "tair_dry_bulb_c",
    ...
    rlang::check_installed(
      c("furrr", "xgboost", "zoo", "MASS"),
      reason = "to run sus_climate_fill_inmet()"
    )
    set.seed(42)

    # ===========================================================
    # MESSAGES + SOURCE + COLUMN DETECTION
    # ===========================================================
    ...
    # ===========================================================
    # RESOLVE target_var  ("all"  →  known vars present in df)
    # ===========================================================
    non_data_cols <- c(datetime_col, station_col)

    if (identical(target_var, "all")) {
      target_var <- intersect(.known_climate_vars, colnames(df))
      target_var <- setdiff(target_var, non_data_cols)

      if (length(target_var) == 0) {
        cli::cli_abort(
          paste0(
            "target_var = 'all' was requested but none of the known climate ",
            "variables were found in the data.\n",
            "Known variables: ",
            paste(.known_climate_vars, collapse = ", ")
          )
        )
      }
      ...
      if (verbose) {
        cli::cli_alert_info(
          sprintf(
            switch(
              lang,
              "pt" = "target_var = 'all': %d variavel(is) encontrada(s): {.field %s}",
              "es" = "target_var = 'all': %d variable(s) encontrada(s): {.field %s}",
              "target_var = 'all': %d variable(s) found: {.field %s}"
            ),
            length(target_var),
            paste(target_var, collapse = ", ")
          )
        )
      }
    }
    ...
    # ===========================================================
    # VALIDATE  target_var  against df columns
    # ===========================================================
    missing_targets <- setdiff(target_var, colnames(df))
    if (length(missing_targets) > 0) {
      cli::cli_abort(
        sprintf(
          "Target variable(s) not found in data: %s",
          paste(missing_targets, collapse = ", ")
        )
      )
    }

    # Warn about any targets not in the canonical known list
    unknown_targets <- setdiff(target_var, .known_climate_vars)
    if (length(unknown_targets) > 0 && verbose) {
      cli::cli_alert_warning(
        sprintf(
          switch(
            lang,
            "pt" = "Variavel(is) fora da lista conhecida (serao imputadas mesmo assim): %s",
            "es" = "Variable(s) fuera de la lista conocida (se imputaran de todas formas): %s",
            "Variable(s) not in the known list (will be imputed anyway): %s"
          ),
    ...
    # ===========================================================
    # RENAME DATETIME COLUMN FOR INTERNAL CONSISTENCY
    # ===========================================================
    if (datetime_col != "date") {
      df <- df %>% dplyr::rename(date = !!rlang::sym(datetime_col))
    }
    ...
    # ===========================================================
    # EVALUATION MODE  — iterate over each target variable
    # ===========================================================
    if (run_evaluation) {
      eval_results <- stats::setNames(
        vector("list", length(target_var)),
        target_var
      )
      ...
      for (var in target_var) {
        if (verbose) {
          cli::cli_h3(
            sprintf(
              switch(
                lang,
                "pt" = "Avaliando performance: {.field %s}  (%d/%d)",
                "es" = "Evaluando desempeno: {.field %s}  (%d/%d)",
                "Evaluating model performance: {.field %s}  (%d/%d)"
              ),
              var,
              which(target_var == var),
              length(target_var)
            )
          )
        }

        # Per-variable quality filter
        station_stats_v <- df %>%
          dplyr::group_by(!!rlang::sym(station_col)) %>%
          dplyr::summarise(
            total_rows = dplyr::n(),
            missing_rows = sum(is.na(!!rlang::sym(var))),
            missing_pct = .data$missing_rows / .data$total_rows,
            .groups = "drop"
          )

        valid_stations_v <- station_stats_v %>%
          dplyr::filter(.data$missing_pct <= quality_threshold) %>%
          dplyr::pull(!!rlang::sym(station_col))

        if (length(valid_stations_v) == 0) {
          if (verbose) {
            cli::cli_alert_warning(
              sprintf(
                "Skipping '%s': no station passed quality threshold.",
                var
              )
            )
          }
        ...
        eval_results[[var]] <- .evaluate_single_station(
          df = df_filtered_v,
          target_var = var,
          station_col = station_col,
          gap_percentage = gap_percentage,
          # parallel plan is already set above — pass parallel = FALSE to
          # avoid a redundant nested plan() call inside the helper
          parallel = FALSE,
          workers = workers,
          lang = lang,
          verbose = verbose
        ...
        if (verbose) {
          cli::cli_alert_success(sprintf("Evaluation complete: %s", var))
        }
      }

      # Remove any NULLs from skipped variables
      eval_results <- Filter(Negate(is.null), eval_results)
      return(eval_results)
    }

    # ===========================================================
    # PRODUCTION MODE  — iterate over each target variable
    # ===========================================================
    if (verbose) {
      cli::cli_alert_info(messages$processing_stations)
    }

    # Keep original column names for the final keep_features logic
    original_cols <- colnames(df)
    all_impute_flag_cols <- character(0)
    imputation_stats <- list()

    df_filled <- df # accumulate imputed columns in-place

    for (var in target_var) {
      if (verbose && length(target_var) > 1) {
        cli::cli_h3(
          sprintf(
            switch(
              lang,
              "pt" = "Imputando variavel %d/%d: {.field %s}",
              "es" = "Imputando variable %d/%d: {.field %s}",
              "Imputing variable %d/%d: {.field %s}"
            ),
            which(target_var == var),
            length(target_var),
            var
      ...
      bad_stations_v <- station_stats_v %>%
        dplyr::filter(.data$missing_pct > quality_threshold) %>%
        dplyr::pull(!!rlang::sym(station_col))

      if (length(bad_stations_v) > 0 && verbose) {
        cli::cli_alert_warning(
          sprintf(
            messages$skipping_stations,
            length(bad_stations_v),
            round(quality_threshold * 100),
            paste(bad_stations_v, collapse = ", ")
          )
        )
      }
      ...
      if (length(valid_stations_v) == 0) {
        if (verbose) {
          cli::cli_alert_warning(
            sprintf(
              "Skipping '%s': no station passed the quality threshold of %.0f%%.",
              var,
      ...
      # ---- Process stations (parallel plan already active) ----
      stations_list <- df_var %>%
      ...
      df_var_filled <- dplyr::bind_rows(results_list)
      ...
      all_impute_flag_cols <- c(all_impute_flag_cols, flag_col)

      # ---- Merge imputed column + flag back into df_filled ----
      # Ensure flag column exists in the target dataframe first
      if (!flag_col %in% colnames(df_filled)) {
        df_filled[[flag_col]] <- FALSE
      }

      # Update with imputed values and flag status
      ...
      df_filled <- df_filled %>%
        dplyr::rows_update(
          df_var_filled %>%
            dplyr::select(dplyr::all_of(c("date", station_col, var, flag_col))),
          by = c("date", station_col),
          unmatched = "ignore"
        )

      # Preencher NAs residuais na flag
      df_filled[[flag_col]][is.na(df_filled[[flag_col]])] <- FALSE
      # ---- Per-variable stats ----
      n_imp <- sum(df_filled[[flag_col]], na.rm = TRUE)
      n_tot <- nrow(df_filled)
      imputation_stats[[var]] <- list(
        n_imputed = n_imp,
        n_total = n_tot,
        imputation_rate = n_imp / n_tot
      )
      ...
      if (verbose) {
        cli::cli_alert_success(
          sprintf(
            switch(
              lang,
              "pt" = "Imputacao concluida para {.strong %s}: {.val %d} linha(s) de {.val %d} ({.strong %.2f%%}).",
              "es" = "Imputacion completada para {.strong %s}: {.val %d} fila(s) de {.val %d} ({.strong %.2f%%}).",
              "Imputation completed for {.strong %s}: {.val %d} row(s) out of {.val %d} ({.strong %.2f%%})."
            ),
            var,
            n_imp,
            n_tot,
    ...
    # ===========================================================
    if (!keep_features) {
      # Keep original columns + all imputation flag columns + date
      cols_to_keep <- unique(c(original_cols, all_impute_flag_cols, "date"))
      # Use datetime_col name (already restored above)
      if (datetime_col != "date") {
        cols_to_keep <- gsub("^date$", datetime_col, cols_to_keep)
      }
    ...
    if (!inherits(df_filled, "climasus_df")) {
      meta <- list(
        ...
        temporal = list(
          start = min(df_filled[[date_col_final]], na.rm = TRUE),
          end = max(df_filled[[date_col_final]], na.rm = TRUE),
          source = source,
          imputed = TRUE,
  ...
  #' @keywords internal
  #' @noRd
  .detect_station_column <- function(
    df,
    station_col,
    source,
    verbose,
  ...
  .get_fill_gaps_messages <- function(lang = "pt") {
    messages <- list(
      pt = list(
        ...
        source_detected = function(s) {
          sprintf("Fonte de dados detectada: %s", s)
        },
        datetime_detected = function(c) {
          sprintf("Coluna de data/hora detectada: %s", c)
        },
        station_detected = function(c) {
          sprintf("Coluna de estacao detectada: %s", c)
        },
      ...
      en = list(
        ...
        datetime_detected = function(c) {
          sprintf("Datetime column detected: %s", c)
        },
        station_detected = function(c) {
          sprintf("Station column detected: %s", c)
        },
      ...
      es = list(
        ...
        source_detected = function(s) {
          sprintf("Fuente de datos detectada: %s", s)
        },
        datetime_detected = function(c) {
          sprintf("Columna de fecha/hora detectada: %s", c)
        },
        station_detected = function(c) {
          sprintf("Columna de estacion detectada: %s", c)
        },
  ...
  .process_single_station_xgboost2 <- function(
    ...
    detected_col <- names(station_df)[sapply(station_df, function(x) {
      inherits(x, c("Date", "POSIXt"))
    })][1]

    if (!is.na(detected_col) && detected_col != "date") {
      station_df <- station_df %>%
        dplyr::rename(date = !!rlang::sym(detected_col))
    }
    ...
    if (length(train_idx) < 30) {
      if (verbose) {
        cli::cli_alert_warning(
          "Insufficient training data ({length(train_idx)} obs), skipping"
        )
      }
      return(station_df)
    }
    ...
    if (transform_type != "none") {
      if (transform_type %in% c("log", "boxcox")) {
        ...
        if (transform_type == "log") {
          ...
          y_train <- if (abs(lambda) < 1e-6) {
            log(y_train_shifted)
          } else {
            (y_train_shifted^lambda - 1) / lambda
          }
      ...
      if (verbose) {
        cli::cli_alert_success(
          "Transformation applied: {.field {transform_type}} | Shift: {round(transform_params$shift, 4)}"
        )
      }
    }
    ...
    model <- tryCatch(
      {
        ...
        X_train <- X_train[,
          colSums(is.na(X_train)) < nrow(X_train),
          drop = FALSE
        ]
        ...
        if (transform_type != "none") {
          if (transform_type == "log") {
            ...
            if (abs(transform_params$lambda) < 1e-6) {
              ...
              preds <- (preds * transform_params$lambda + 1)^(1 /
                transform_params$lambda) -
...
#' Smooth Extreme Predictions
  ...
  #' @keywords internal
  #' @noRd
  .smooth_extreme_predictions <- function(
    preds,
    train_vals,
    iqr_multiplier = 5
  ) {
    ...
    if (length(extreme_idx) > 0) {
      ...
      preds[extreme_idx] <- alpha *
        train_mean +
        (1 - alpha) * preds[extreme_idx]
    }
...
#' Create Time-Based Features
  ...
  create_lag_features <- function(
    ...
    if (!is.null(group_cols) && datetime_col %in% colnames(data)) {
      data <- data %>%
        dplyr::arrange(!!!rlang::syms(c(group_cols, datetime_col)))
    ...
    if (na_action == TRUE) {
      ...
      is_num <- vapply(
        created_cols,
        function(x) is.numeric(result_data[[x]]),
        FUN.VALUE = logical(1)
      )
      ...
      if (lang == "pt") {
        if (abs(r_squared) < 0.3) {
          cli::cli_alert_danger(
            "R\u00b2 muito baixo. O modelo n\u00e3o est\u00e1 explicando a variabilidade dos dados."
          )
        } else if (r_squared < 0.6) {
          cli::cli_alert_warning(
            "R\u00b2 moderado. O modelo explica parcialmente os dados."
          )
        } else if (r_squared > 0.9) {
          cli::cli_alert_success(
            "R\u00b2 excelente! O modelo explica muito bem os dados."
          )
        }

        if (abs(slope_bias - 1) > 0.2 && !is.na(slope_bias)) {
          cli::cli_alert_warning(
            "Vi\u00e9s de inclina\u00e7\u00e3o detectado. Slope = {round(slope_bias, 3)} (ideal = 1.0)"
          )
        }
        ...
        if (smape < 10) {
          ...
          cli::cli_alert_danger(
            "sMAPE alto: {round(smape, 1)}%. Erro percentual significativo."
          )
        }
      } else if (lang == "en") {
        if (abs(r_squared) < 0.3) {
          cli::cli_alert_danger(
            "Very low R\u00b2. Model is not explaining data variability."
          )
        } else if (r_squared < 0.6) {
          cli::cli_alert_warning(
            "Moderate R\u00b2. Model partially explains the data."
          )
        } else if (r_squared > 0.9) {
          cli::cli_alert_success(
            "Excellent R\u00b2! Model explains data very well."
          )
        }

        if (abs(slope_bias - 1) > 0.2 && !is.na(slope_bias)) {
          cli::cli_alert_warning(
            "Slope bias detected. Slope = {round(slope_bias, 3)} (ideal = 1.0)"
          )
        }
        ...
        if (smape < 10) {
          ...
          cli::cli_alert_danger(
            "High sMAPE: {round(smape, 1)}%. Significant percentage error."
          )
        }
      }
    }
  ...
  .introduce_nan <- function(
    ...
    if (gap_mechanism == "MCAR") {
      ...
      if (length(missing_vars) > 0) {
        cli::cli_abort(
          "mar_depends_on variables not found: {paste(missing_vars, collapse = ', ')}"
        )
      }
...
#' Evaluate Single Station Imputation Performance
  ...
  .evaluate_single_station <- function(
    ...
    mar_depends_on = NULL,
    remove_outliers = TRUE,
    parallel = TRUE, # kept for signature compatibility; plan is owned by caller
    workers = NULL, # kept for signature compatibility; ignored here
    lang = "pt",
    seed = 42,
    verbose = verbose
  ) {
    ...
    # ============================================================================
    # APLICAR  — parallel plan is already set (or not) by the calling function;
    # we always use furrr::future_map so it respects whatever plan is active.
    # ============================================================================
    ...
    comparison_data <- original_data[gap_indices, ] %>%
      dplyr::select(
        ...
        .join_key,
        true_value = dplyr::all_of(target_var)
      ) %>%
      dplyr::left_join(
        filled_data %>%
          dplyr::select(.join_key, pred_value = dplyr::all_of(target_var)),
        by = ".join_key"
      )

    my_stations <- unique(
      comparison_data %>% dplyr::select(dplyr::all_of(station_col))
    )
