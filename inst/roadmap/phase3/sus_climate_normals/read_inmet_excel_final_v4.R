...
.read_inmet_excel <- function(file_path, var_code) {
  ...
  tryCatch({
    ...
    for (i in 4:ncol(data)) {
      ...
      # Extract only the decade number (1, 2, or 3) from the decade indicator
      # Examples: "1º DEC" -> "1", "2º DEC" -> "2", "3º DEC" -> "3"
      decade_clean <- decade_indicator %>%
        stringi::stri_trans_general("Latin-ASCII") %>%
        stringi::stri_replace_all_regex("[^0-9]", "") %>%
        stringi::stri_sub(1, 1)  # Extract only the first digit
    ...
    # Now we can safely split on the last underscore since decade is just 1 digit
    data_long <- data_long %>%
      tidyr::separate(
        mes_decada,
        into = c("mes", "decada"),
