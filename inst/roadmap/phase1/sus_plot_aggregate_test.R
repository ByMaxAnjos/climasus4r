...
#
# EPIDEMIOLOGICAL RATIONALE FOR SMOOTH MODEL CHOICE:
#
# Original: GAM with cs (cubic regression spline)
# Problem:  geom_smooth() aggregates raw daily/municipality rows before fitting,
#           so it smooths the stacked incidence values rather than a proper
#           aggregate. For a state-level trend we must pre-aggregate first.
#
# Recommended smoother: LOESS with carefully chosen span OR
#                        GAM with tp (thin-plate) spline on aggregated data.
#
# Why NOT plain GAM cs on raw data:
#   - Daily municipality data is zero-inflated and highly heteroskedastic
#   - Fitting on individual municipality rows means N is huge → over-narrow CI
#   - cs spline can produce artefacts at series edges (Ramsay 2002)
#
# Why LOESS (span ≈ 0.15–0.20) on pre-aggregated weekly medians:
#   - Resistant to outlier outbreak spikes (uses local medians)
#   - No distributional assumption → appropriate for count/rate data
#   - span controls how many epidemic cycles are captured; 0.15 for 8-year
#     series recovers annual seasonality without over-smoothing
#
# Why log1p Y-axis:
#   - Dengue incidence is extremely right-skewed: capital cities (Manaus,
#     Porto Velho) have peak incidences >5000/100k while small municipalities
#     are near zero most of the year. Linear scale collapses small-state
#     variation into noise. log1p preserves zero values.
#   - Consistent with WHO and PAHO dengue reporting standards.
#
# =============================================================================
...
# ── 0. Pre-aggregate to WEEKLY STATE MEDIAN ──────────────────────────────────
# This is the critical correction vs. the original code.
# geom_smooth on raw rows would weight high-incidence outbreak weeks equally
# with zero days, producing a systematically upward-biased trend line.
# Median is preferred over mean for epidemiological rate data (resistant to
# super-spreader events and reporting lags).
...
plot_scientific <- ggplot(df_plot, aes(x = epi_week)) +
  ...
  # ── 4g. Labels ───────────────────────────────────────────────────────────
  labs(
    title    = "Spatio-temporal Dynamics of Dengue Fever in the Brazilian Amazon (2025)",
    subtitle = paste0(
...
ggsave(
  filename = "dengue_amazon_incidence_final.png",
  plot     = plot_scientific,
  width    = 10,       # Proporção ideal para slides
  height   = 5.62,     # (16:9 ratio)
  units    = "in",     # Polegadas facilitam o controle de escala no ggplot
  dpi      = 600,      # Aumentar para 600 garante nitidez máxima em telas grandes
