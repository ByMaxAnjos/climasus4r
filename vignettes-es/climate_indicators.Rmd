---
title: "Índices Bioclimáticos para Salud Pública"
subtitle: "Teoría y Pipeline INMET → Relleno → Indicadores Térmicos | `sus_climate_compute_indicators()`"
author:
  - name: "Max Anjos"
  - name: "João Gobo"
  - name: "Thauã Menezes"
...
Este documento demuestra el pipeline completo del paquete **climasus4r** para la
generación de índices bioclimáticos a partir de datos horarios INMET: descarga →
relleno de lagunas → cálculo de 15 indicadores → visualización y categorización
regional de riesgo térmico.
:::
...
## ¿Por qué índices bioclimáticos?

La temperatura del aire (`T`) por sí sola no representa adecuadamente el estrés térmico humano. El sistema termorregulador responde a un **balance energético complejo** que integra:

*   **Temperatura del aire** (`T`, °C)
*   **Humedad relativa** (`RH`, %): afecta la eficiencia de la evaporación del sudor.
*   **Velocidad del viento** (`WS`, m/s): modifica la capa límite de convección.
*   **Radiación solar** (`SR`, W/m²): añade carga térmica radiante.

Índices como WBGT, UTCI y PET traducen esta interacción en un valor escalar con significado fisiológico directo, siendo herramientas estándar en salud ocupacional, vigilancia de olas de calor y planificación urbana.

## 📊 Indicadores Bioclimáticos y de Estrés Térmico

| Código | Nombre | Variables de Entrada | Dominio de Validez | Referencia Principal |
|:---|:---|:---|:---|:---|
...
| `diurnal_range` | Amplitud Térmica Diurna | T (min, max) | General | - |
| `vapor_pressure` | Presión de Vapor Actual | T, RH | General | Magnus-Tetens (1930) |
| `heat_stress_risk` | Riesgo de Estrés Térmico | WBGT, UTCI | Categórico | OSHA/ACGIH (2017) |
| `koppen_humidity` | Clase de Humedad (Köppen) | RH | Categórico | Köppen (1936) |
...
Modelo implementado para ambiente exterior bajo carga solar directa.
...
*   **T<sub>db</sub>** = Temperatura de bulbo seco (°C).
*   **T<sub>nwb</sub>** = Temperatura de bulbo húmedo natural (°C). Para robustez numérica, se utiliza el promedio de dos estimaciones:
    ...
    *   Donde `e_a` es la presión de vapor actual (kPa), calculada a partir de T y RH.
*   **T<sub>g</sub>** = Temperatura de globo (°C), que modela la carga radiante:
    *   `T_g = T + 0.0144 * max(G_0, 0)^0.6 / WS^0.2 - 2.0`
    *   `G_0` es la irradiancia solar en W/m², convertida de SR (kJ/m²) mediante `SR / 3.6`.

**Ponderación:** Los coeficientes 0.7, 0.2 y 0.1 representan la contribución relativa del enfriamiento evaporativo (bulbo húmedo), de la carga radiante (globo) y del calor sensible (bulbo seco) para el estrés térmico en trabajadores al aire libre.
</div>

### 2. *Heat Index* (HI)

Implementación del modelo de regresión polinomial del *National Weather Service* (NWS), con ajustes para condiciones de humedad extrema.
...
El valor final se convierte a Celsius: `HI = (HI°F - 32) * 5/9`.

**Umbral de Validez Adaptado por Bioma:**
*   Estándar: T ≥ 26.7 °C.
*   Regiones tropicales (Amazonía, Mata Atlántica): T ≥ 24.0 °C, debido a la aclimatación de las poblaciones locales.

**Ajustes Post-Cálculo:**
*   Si RH < 13% y 26.7 ≤ T ≤ 44.4 °C: `HI = HI - ((13 - RH)/4) * sqrt((17 - |T - 95|) / 17)`.
*   Si RH > 85% y 26.7 ≤ T ≤ 30.6 °C: `HI = HI + ((RH - 85)/10) * ((87 - T)/5)`.
</div>

### 3. *Universal Thermal Climate Index* (UTCI)

Aproximación polinomial del modelo de referencia Fiala, optimizada para aplicaciones operacionales a gran escala.

<div class="equation-card">

1.  **Temperatura Radiante Media (T<sub>mrt</sub>):** `T_mrt = T + 0.07 * G_0 - 1.5` (aproximación para radiación de onda corta).
2.  **Presión de Vapor (VP, kPa):** `VP = 0.6108 * exp(17.27 * T / (T + 237.3)) * (RH / 100)` @Tetens1930.
3.  **Velocidad del Viento a 10 m (WS<sub>10m</sub>):** `WS_10m = max(WS * 1.5, 0.5)` (conversión de la altura estándar de 2m a 10m).

**UTCI (°C) = T + 0.048 * ((VP*1000) - 1000)/100 - 0.29 * ln(WS_10m + 0.1) + 0.016 * ΔT_mrt + 0.010 * (ΔT_mrt²/10) - 6e-5 * ΔT_mrt² * WS_10m + δ<sub>regional</sub>**

Donde `ΔT_mrt = T_mrt - T`. El término `δ<sub>regional</sub>` es una corrección empírica calibrada para biomas brasileños.
</div>

### 4. Grados-Día (CDD, HDD, GDD)
...
*   **GDD** (*Growing Degree Days*): `GDD = max(0, min(T, T_upper) - T_base_gdd)`. Base regional: 8–15 °C. Límite superior regional: 30–35 °C.
...
## Paso 3 · Cálculo de los Indicadores
...
## 4.1 Visión General de los Indicadores
...
*Nota: el bloque anterior usa `ggridges`; instálelo con `install.packages("ggridges")` si es necesario.*
...
## 4.4 Categorías de Riesgo Regional
...
## 4.8 Clasificaciones Categóricas
...
## Efecto del Umbral Regional en el HI
...
# Incertidumbre y Flags de Confianza

## Estimativa de Incertidumbre (Monte Carlo)
...
## Integración completa en el pipeline SUS
...
## Umbrales Personalizados para Investigación
...
## Solo Indicadores Seleccionados
...
# Tabla de Referencia de Categorías de Riesgo

### 🌡️ Clasificación de Riesgo Térmico por Índices Bioclimáticos

| Índice | Categoría | Umbral (°C) | Significado Fisiológico | Fuente |
|--------|----------|-------------|--------------------------|-------|
| WBGT | Ninguno | < 20 | Sin riesgo térmico perceptible | OSHA/ACGIH |
| WBGT | Bajo | 20–25 | Estrés leve en actividad intensa | OSHA/ACGIH |
| WBGT | Moderado | 25–28 | Pausas recomendadas para trabajadores | OSHA/ACGIH |
| WBGT | Alto | 28–30 | Riesgo real de trastornos térmicos | OSHA/ACGIH |
| WBGT | Muy Alto | 30–33 | Reducción severa del tiempo de trabajo | OSHA/ACGIH |
| WBGT | Extremo | > 33 | Riesgo de colapso por calor | OSHA/ACGIH |
| UTCI | Frío extremo | < −27 | Hipotermia inminente | Bröde et al. (2012) |
| UTCI | Neutro | 9–26 | Zona de confort térmico | Bröde et al. (2012) |
| UTCI | Calor leve | 26–32 | Estrés leve por calor | Bröde et al. (2012) |
| UTCI | Calor fuerte | 32–38 | Estrés moderado por calor | Bröde et al. (2012) |
| UTCI | Calor extremo | > 46 | Estrés térmico extremo | Bröde et al. (2012) |
| PET | Frío extremo | < 4 | Disconfort térmico por frío | Matzarakis et al. (1999) |
| PET | Confort | 18–23 | Zona térmica neutra | Matzarakis et al. (1999) |
| PET | Calor leve | 23–29 | Leve disconfort por calor | Matzarakis et al. (1999) |
| PET | Calor extremo | > 41 | Estrés térmico muy fuerte | Matzarakis et al. (1999) |
| HI | Precaución | 27–32 | Fatiga posible con exposición prolongada | NWS |
| HI | Peligro extremo | > 54 | Insolación altamente probable | NWS |
---

# Consideraciones Científicas y Limitaciones

::: {.warning-box}
**⚠️ Limitaciones Importantes a Reportar**

1. **UTCI y PET son aproximaciones** — el UTCI completo (Fiala) exige un polinomio de
   210+ términos; esta implementación puede divergir 3–5°C en condiciones extremas
   (Bröde et al., 2012). Para estudios epidemiológicos críticos, use `reticulate` +
   `pythermalcomfort` para el UTCI completo.

2. **Radiación solar nocturna = 0** — los valores NA en `sr_kj_m2` durante la noche
   se imputan como 0 (sin carga solar). El WBGT nocturno refleja solo
   temperatura, humedad y viento — físicamente correcto.

3. **PET y vestimenta** — el modelo simplificado asume vestimenta media estacional
   (clo 0.5–0.9); poblaciones con hábitos muy distintos (ej.: mineros,
   trabajadores rurales) requieren calibración específica.

4. **Las adaptaciones regionales son empíricas** — los desplazamientos (+1.5°C Amazonía,
   −0.5°C Caatinga, etc.) reflejan consenso bibliográfico; se recomienda validación local
   contra mediciones de globo termómetro para publicaciones.

5. **Datos INMET** — las estaciones automáticas presentan problemas de mantenimiento;
   el relleno de lagunas con XGBoost reduce las lagunas pero no elimina la incertidumbre de
   medición. Use `compute_uncertainty = TRUE` para propagar esta incertidumbre.
:::
...
¿Tienes alguna idea de mejora o encontraste algún error? Haz clic en el botón de abajo para abrir un nuevo *issue* en GitHub y compartir tus sugerencias directamente con nosotros.
