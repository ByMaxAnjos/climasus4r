...
author:
  - name: "Max Anjos , João Gobo" & " Thauã Menezes"
...
A temperatura do ar (`T`) isoladamente não representa adequadamente o estresse térmico humano. O sistema termorregulatório responde a um **balanço energético complexo** que integra:
...
*   **T<sub>nwb</sub>** = Temperatura de bulbo úmido natural (°C). Para robustez numérica, utiliza-se a média de duas estimações:
...
*   **T<sub>g</sub>** = Temperatura de globo (°C), que modela a carga radiante:
...
**Limiar de Validade Adaptado por Bioma:**
*   Padrão: T ≥ 26.7 °C.
*   Regiões tropicais (Amazônia, Mata Atlântica): T ≥ 24.0 °C, devido à aclimatação das populações locais.

**Ajustes Pós-Cálculo:**
*   Se RH < 13% e 26.7 ≤ T ≤ 44.4 °C: `HI = HI - ((13 - RH)/4) * sqrt((17 - |T - 95|) / 17)`.
*   Se RH > 85% e 26.7 ≤ T ≤ 30.6 °C: `HI = HI + ((RH - 85)/10) * ((87 - T)/5)`.
</div>
...
1.  **Temperatura Radiante Média (T<sub>mrt</sub>):** `T_mrt = T + 0.07 * G_0 - 1.5` (aproximação para radiação de ondas curtas).
2.  **Pressão de Vapor (VP, kPa):** `VP = 0.6108 * exp(17.27 * T / (T + 237.3)) * (RH / 100)` @Tetens1930.
3.  **Velocidade do Vento a 10 m (WS<sub>10m</sub>):** `WS_10m = max(WS * 1.5, 0.5)` (conversão da altura padrão de 2m para 10m).
