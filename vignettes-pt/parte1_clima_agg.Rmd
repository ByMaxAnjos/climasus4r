...
author:
  - name: "Max Anjos", "Eucilene Santana" & "Thauã Menezes"
...
<div class="equation">
**Modelo Matemático:** $\log(E) = \alpha + \sum_{j=0}^{L} f(x_{t-j}, j) + \text{covariáveis}$
</div>
...
<div class="equation">
**Modelo:** $Y_t \leftarrow \frac{1}{(W_2 - W_1) + 1} \sum_{i=t-W_2}^{t-W_1} X_i$
</div>
...
<div class="equation">
**Contagem de Excedência:** $N_{\text{exc}} = \sum_{d=0}^{W} \mathbf{1}[X_{t-d} > \tau]$
</div>
...
<div class="equation">
**Média Ponderada:** $\bar{X}_{\text{weighted}} = \frac{\sum_{d=0}^{W} w_d \cdot X_{t-d}}{\sum_{d=0}^{W} w_d}$
</div>

| Tipo de Decaimento | Função | Meia-vida | Aplicação |
|--------------------|--------|-----------|------------|
| Linear | $w_d = 1 - \frac{d}{W+1}$ | Variável | Efeitos genéricos |
| Exponencial | $w_d = e^{-\lambda d}$ | $\ln(2)/\lambda$ | Poluição do ar |
| Gaussiano | $w_d = e^{-(d-\mu)^2/(2\sigma^2)}$ | $\sigma\sqrt{2\ln2}$ | Período de incubação |
...
1.  **Armstrong, B. (2006)**. Models for the relationship between ambient temperature and daily mortality. *Epidemiology*, 17(6), 624-631.

2.  **Basu, R., & Samet, J. M. (2002)**. Relation between elevated ambient temperature and mortality: a review of the epidemiologic evidence. *Epidemiologic Reviews*, 24(2), 190-202.

3. **Bhaskaran, K., et al. (2013)**. Time series regression studies in environmental epidemiology. *International Journal of Epidemiology*, 42(4), 1187-1195.
...
5.  **Gasparrini, A., et al. (2015)**. Mortality risk attributable to high and low ambient temperature: a multicountry observational study. The Lancet, 386(9991), 369-375.

6. **Lowe, R., et al. (2021)**. Combined effects of hydrometeorological hazards and urbanisation on dengue risk in Brazil: a spatiotemporal modelling study. *The Lancet Planetary Health*, 5(4), e209-e219.
